    ...
    <div className="mx-auto w-[min(1100px,92vw)] space-y-14 py-8">
      <section className="grid gap-8 md:grid-cols-2">
        ...
        <div className="self-center">
          ...
          {exhibitions.current.catalog ? <a href={exhibitions.current.catalog} className="mt-6 inline-block border border-ink/20 px-5 py-3 text-xs uppercase tracking-[0.2em]">Download</a> : null}
      ...
      <section>
        ...
        <p className="section-label">Installation + Available Works</p>
        <div className="mt-5 grid gap-5 md:grid-cols-3">
          {featuredArtworks.map((work) => (
            <article key={work.id} className="border border-ink/10 p-3 text-sm">
              <div className="relative h-64 overflow-hidden"><Image src={work.image} alt={work.title} fill className="object-cover" /></div>
      ...
      <section>
        ...
        <blockquote className="mt-6 border-l-2 border-terracotta pl-4 text-sm italic text-ink/80">{exhibitions.quote}</blockquote>
        <Link href="/contact" className="mt-6 inline-block border border-ink/20 px-5 py-3 text-xs uppercase tracking-[0.2em]">Book a private viewing</Link>
        <div className="mt-5 grid gap-5 md:grid-cols-2">
          {exhibitions.past.map((item) => (
