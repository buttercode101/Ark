    ...
    <div className="space-y-24 pb-8">
      <section className="relative min-h-[84vh] overflow-hidden">
        ...
        <div className="relative z-10 mx-auto flex min-h-[84vh] w-[min(1100px,92vw)] flex-col justify-end pb-16 text-bg">
          ...
          <h1 className="editorial-title mt-3 max-w-3xl text-5xl md:text-7xl">
            {exhibitions.current.title}
          </h1>
          <p className="mt-3 max-w-2xl text-bg/90">{exhibitions.current.description}</p>
          <Link
            href="/exhibitions"
            className="mt-7 w-fit border border-bg/50 px-6 py-3 text-sm uppercase tracking-[0.22em] transition-colors hover:bg-bg hover:text-ink"
          >
            View Exhibition
          </Link>
        </div>
      ...
      <AnimatedSection>
        <section className="mx-auto w-[min(1100px,92vw)] rounded-sm bg-mist p-10 text-center">
          ...
          <h2 className="editorial-title mt-4 text-4xl">{gallery.about}</h2>
          <Link
            href="/visit"
            className="mt-8 inline-block border border-ink/20 px-6 py-3 text-xs uppercase tracking-[0.25em]"
          >
            Plan your visit
          </Link>
        </section>
